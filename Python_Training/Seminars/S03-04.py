...
def randomize_it(n):
    count, length = n[0], n[1]
    num = time.time() # узнаем текущее время в секундах
    num = int(str(num%1)[2:]) # получаем целое число из дробной части текущего времени
    lst = []
    while len(lst) != count:
        num = num<<1 # сдвигаем число на 1 бит
        nw_n = int(str(num)[-(length+1):-1]) # считываем последние цифры числа, взависимости от заданного размера
        if nw_n >= 10**(length-1): lst.append(nw_n) # если число меньше длины (из-за нуля), то оно не добавиться
    return lst
